...
def load_and_augment_from_url():
    ...
    # --- FIX #1: Re-instating 'CustomerID' into the dropna command as it exists in your data ---
    df.dropna(subset=['StockCode', 'Description', 'Customer ID'], inplace=True)
    ...
    # --- FIX #2: Changing 'UnitPrice' to 'Price' to match your column name ---
    sku_agg = recent_sales_df.groupby(['StockCode', 'Description']).agg(
        total_quantity_sold=('Quantity', 'sum'),
        Price=('Price', 'mean') # This now correctly uses your 'Price' column
    ).reset_index()
    ...
    # --- Intelligent Augmentation (UNCHANGED) ---
    ...
    sku_df['Supplier'] = assignments.str[0]
    sku_df['Brand'] = assignments.str[1]
...
st.subheader("Live Data from Online Source")
...
if processed_df is not None:
    st.sidebar.header("Workbench Filters")
    selected_suppliers = st.sidebar.multiselect("Filter by Supplier:", options=sorted(processed_df['Supplier'].unique()), default=sorted(processed_df['Supplier'].unique()), key="supplier_filter")
    selected_brands = st.sidebar.multiselect("Filter by Brand:", options=sorted(processed_df['Brand'].unique()), default=sorted(processed_df['Brand'].unique()), key="brand_filter")
    selected_risk = st.sidebar.multiselect("Filter by Risk Level:", options=processed_df['Risk'].unique(), default=sorted(processed_df['Risk'].unique()), key="risk_filter")
    show_all = st.sidebar.checkbox("Show all SKUs (not just reorders)", value=False, key="show_all_filter")
    include_legacy = st.sidebar.checkbox("Include Legacy (Inactive) SKUs", value=False, key="legacy_filter")
    ...
    if not show_all:
        filtered_df = filtered_df[filtered_df['AI_Reorder?'] | filtered_df['BO4_Advice?']]
    active_df = filtered_df[filtered_df['Is_Active']]; legacy_df = filtered_df[~filtered_df['Is_Active']]
    workbench_df = filtered_df if include_legacy else active_df
    ...
    if st.session_state.active_tab == "📈 Dashboard":
        st.header("Overall Performance Dashboard"); st.markdown("Metrics below are for **active SKUs** only.")
        active_kpi_df = processed_df[processed_df['Is_Active']]
        col1, col2, col3 = st.columns(3); reorders_count = active_kpi_df[active_kpi_df['AI_Reorder?']].shape[0]; value_at_risk = (active_kpi_df[active_kpi_df['Risk'] == '🔴 High']['AI_Reorder_Qty'] * active_kpi_df['Price']).sum(); overstock_value = (active_kpi_df[active_kpi_df['Current_Stock'] > (active_kpi_df['Target_DOS'] * active_kpi_df['p50'])]['Current_Stock'] * active_kpi_df['Price']).sum()
        col1.metric("AI Reorders Required", f"{reorders_count} SKUs"); col2.metric("Retail Value at Risk (High Risk)", f"€{value_at_risk:,.0f}"); col3.metric("Potential Overstock Retail Value", f"€{overstock_value:,.0f}")
        ...
        def handle_row_selection():
            if st.session_state.workbench_selection["selection"]["rows"]:
                selected_row_index = st.session_state.workbench_selection["selection"]["rows"][0]
                st.session_state.selected_sku = workbench_df.iloc[selected_row_index]['SKU']
                st.session_state.active_tab = "🔍 SKU Investigation"
        st.dataframe(workbench_df[['SKU', 'Description', 'Supplier', 'Price', 'Current_Stock', 'Days_Out_Of_Stock', 'AI_Reorder_Qty', 'Risk', 'Is_Active']], on_select=handle_row_selection, selection_mode="single-row", use_container_width=True, hide_index=True, key="workbench_selection")
    elif st.session_state.active_tab == "🔍 SKU Investigation":
        st.header("SKU Drilldown & Investigation")
        options = workbench_df['SKU'].unique()
        ...
        if not workbench_df.empty:
            sku_to_inspect = st.selectbox("Select an SKU to investigate:", options=options, index=default_index, format_func=lambda x: f"{x} - {processed_df.loc[processed_df['SKU'] == x, 'Description'].iloc[0]}", key="sku_selector")
            st.session_state.selected_sku = sku_to_inspect
            sample_sku_data = processed_df.loc[processed_df['SKU'] == sku_to_inspect].iloc[0]
